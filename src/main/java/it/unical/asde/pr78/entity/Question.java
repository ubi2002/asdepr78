...
public final class Question extends BaseEntity {
	...
	@Column(name = "`is_required`", nullable = false, columnDefinition = "TINYINT(1) UNSIGNED")
	@NotNull
	@ColumnDefault("1")
	private Boolean isRequired = true;

	@Column(name = "`point`", nullable = false, columnDefinition = "TINYINT UNSIGNED")
	...
	public String getAnswerValue() {
		...
		case TYPE_DROPDOWN:
		default:
			List<Long> choiceIds = Arrays
					.asList(getAnswers().stream().map(a -> Long.valueOf(a.getValue())).toArray(Long[]::new));
	...
	public List<Long> getCorrectChoiceIds() {
		return Arrays.asList(getCorrectChoices().stream().map(Choice::getId).toArray(Long[]::new));
	}

	public List<Choice> getCorrectChoices() {
		return Arrays.asList(getChoices().stream().filter(Choice::getCorrect).toArray(Choice[]::new));
	}
	...
	public String toString() {
		return "Question{" + "id=" + getId() + ", title='" + title + '\'' + ", desc='" + desc + '\'' + ", type=" + type
				+ ", isRequired=" + isRequired + ", point=" + point + ", choices=" + choices + ", answers=" + answers
