...
public class ExamServiceImpl implements ExamService {
	...
	public List<Exam> findAllAvailableForStudent(List<Submission> submissions) {
		List<Long> submissionIds = Arrays
				.asList(submissions.stream().map(s -> s.getExam().getId()).toArray(Long[]::new));
	...
	private Exam processExamForEditing(User professor, Exam exam, List<Question> questionsToDelete,
			List<Choice> choicesToDelete) throws InvalidExamException {
		...
		if (exam.getId() != null) {
			...
			if (!existedExam.isEditable()) {
				...
				String error = String.format("You can not edit the exam '%s'. Because it is active now.",
						exam.getTitle());
				throw new InvalidExamException(error);
			}
	...
	/**
	 * Save new questions Update the existed questions Removed the lacked question
	 *
	 * @param existedExam
	 * @param exam
	 ...
	 * @throws InvalidExamException
	 */
	private void processQuestionsForExam(Exam existedExam, Exam exam, List<Question> questionsToDelete,
			List<Choice> choicesToDelete) throws InvalidExamException {
				...
				for (Question question : exam.getQuestions()) {
					if (question.getId() != null) {
						...
						String error = String.format(
								"You can not edit the exam '%s'. Because it contains an invalid question '%s'.",
	...
	/**
	 * Save new choices Update the existed choices Removed the lacked choices
	 *
	 * @param exam
			...
			if (existedQuestion.getChoices() == null || existedQuestion.getChoices().isEmpty()) {
				for (Choice choice : question.getChoices()) {
					if (choice.getId() != null) {
						...
						String error = String.format(
								"You can not edit the exam '%s'. Because the question '%s' contains an invalid choice '%s'.",
