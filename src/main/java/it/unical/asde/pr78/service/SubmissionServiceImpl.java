...
public class SubmissionServiceImpl implements SubmissionService {
	...
	/**
	 * Need to check: - Exam should exist - User didn't finish exam - If exam has
	 * been started, the duration from startedAt to now can not be greater or equal
	 * than duration of exam
	 *
	 * @param user
	 * @param id
	 * @return
			...
			if (submission.getStatus() == Submission.STATUS_STARTED) {
				...
				if (now.getTime() > expectedEndTime) {
					String error = String.format(
							"You can not continue the exam '%s'. Because you have started at %s. But you did not complete within %d minutes.",
				...
				if (now.getTime() > exam.getFinishedAt().getTime()
					...
					String error = String.format(
							"You can not continue the exam '%s'. Because you have started at %s. But the exam has been closed at %s.",
							exam.getTitle(), format.format(submission.getStartedAt()),
							format.format(exam.getFinishedAt()));
					throw new InvalidExamException(error);
				}
			} else {
				String error = String.format(
						"You can not continue the exam '%s'. Because the exam has been closed at %s.", exam.getTitle(),
						format.format(submission.getFinishedAt()));
				throw new InvalidExamException(error);
			}
		} else {
	...
	public List<Submission> findAllByExam(Exam exam) {
		List<Integer> availableStatuses = Arrays.asList(Submission.STATUS_SUBMITTED, Submission.STATUS_REVIEWING,
	...
	public Submission findForProfessorToReview(Exam exam, Long submissionId) throws InvalidExamException {
		...
		Submission submission = this.submissionRepository.findByIdAndExamAndStatusIn(submissionId, exam,
				toReviewStatuses);

		if (submission == null) {
			String error = String.format("The submission for exam '%s' has been already reviewed or invalid",
					exam.getTitle());
			throw new InvalidExamException(error);
		}
		...
		for (Answer answer : answers) {
			if (!answer.getReviewed()) {
				String message = String.format(
						"The answers of exam '%s' are not fully reviewed. Please check them before marking as reviewed.",
						exam.getTitle());
	...
	public Submission findReviewing(Exam exam, Long submissionId) throws InvalidExamException {
		Submission submission = this.submissionRepository.findByIdAndExamAndStatus(submissionId, exam,
				Submission.STATUS_REVIEWING);
	...
	public Submission findReviewed(Exam exam, Long submissionId) throws InvalidExamException {
		...
		if (submission == null) {
			String error = String.format("The submission for exam '%s' has been not reviewed or invalid.",
					exam.getTitle());
			throw new InvalidExamException(error);
		}
